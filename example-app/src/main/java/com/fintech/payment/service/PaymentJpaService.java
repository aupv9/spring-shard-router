...
public class PaymentJpaService {
    ...
    public PaymentJpaService(AccountRepository accountRepository,
    ...
    public void processPayment(long accountId, BigDecimal amount, String description) {
        // SELECT FOR UPDATE: lock the row so concurrent transactions serialize on this account
        Optional<Account> accountOpt = accountRepository.findByAccountIdForUpdate(accountId, accountId);
    ...
    public void transferMoney(long fromAccountId, long toAccountId, BigDecimal amount, String description) {
        ...
        // SELECT FOR UPDATE both accounts to prevent concurrent lost updates
        Optional<Account> fromAccountOpt = accountRepository.findByAccountIdForUpdate(fromAccountId, fromAccountId);
        Optional<Account> toAccountOpt = accountRepository.findByAccountIdForUpdate(fromAccountId, toAccountId);
    ...
    /**
     * Check if two accounts are on the same shard using the configured ShardRouter.
     * This correctly handles both HASH and CONSISTENT_HASH strategies.
     */
    private boolean onSameShard(long accountId1, long accountId2) {
        return shardRouter.resolve(accountId1).index() == shardRouter.resolve(accountId2).index();
    }
}